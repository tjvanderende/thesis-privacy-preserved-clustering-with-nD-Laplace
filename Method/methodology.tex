...
To address these limitations, we propose a new \gls{ldp} method called kd-Laplace, utilizing distance to preserve data patterns using the concept of \gls{gi}. \newline
...
\begin{enumerate}
  ...
  \item \textit{RQ3: What is the impact of dataset characteristics on the research conducted for kD-Laplace?} \newline
        ...
        \begin{enumerate}
          \item \textit{Adding remapping based on density improves utility without sacrificing privacy:}
          ...
          \item \textit{The privacy leakage (adversary advantage) and utility increases for more dimensions:}
...
As discussed for research question 3 \todo{Reference}, we omitted the synthetic datasets because they are only used with K-Means.
...
The "minimum points" hyperparameter is calculated using the formula $minPts = n * 2$, where $n$ is the number of dimensions (See Section: \ref{theory:clustering-dbscan}).
...
\begin{enumerate}
  ...
  \item Mechanism utility: Both mechanisms are compared against each other by evaluating the \gls{ami} and sc.
