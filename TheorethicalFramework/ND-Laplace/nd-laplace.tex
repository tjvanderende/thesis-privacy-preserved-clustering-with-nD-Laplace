...
The calculations for this are straightforward. \todo{See link for formulas}
...
This enables efficient searching of records based on space. This is illustrated using a two-dimensional representation based on figure \todo{add figure reference}.
...
Additionally, nearest neighbor (NN) searching is efficient, with a time complexity of $O(\log n)$ \citep{washington_k-d_2002}. \todo{Add a link in the appendix to time complexity}.
\begin{figure}[H]
...
Using this algorithm, we can effectively remap point $z \in Z$ to either $X$ or $G$ based on the closest Euclidean distance.
...
When $\epsilon$ is very low (and thus closer), the data points are more likely to map to the grid surface \todo{Add see link to 3D Laplace}.
...
\end{figure}
The remapping algorithm works on the idea of crowded places \todo{Reference RQ1}, with the intuition that a crowded place leverages indistinguishability by crowdedness.
...
Hence, this is described as $Q_r = B_r \cap Q$ (See section 3.1.3. \todo{add link}).
