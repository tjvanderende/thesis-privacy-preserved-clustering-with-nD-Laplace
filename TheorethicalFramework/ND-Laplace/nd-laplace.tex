...
The theorem on extending $\epsilon$-geo-indistinguishability to $\epsilon-d_x$-privacy is as follows \citep{fernandes_generalised_2019}:
\begin{theorem}
...
\end{theorem}
Because $d_x$ is a pseudo metric, we can also provide $d_E$ to denote the Euclidean distance.
...
\todo[inline]{Needs refactoring with proof and formal representation}
In the previous sections, we introduced the 2D, 3D, and nD Laplace mechanisms.
The current section will be used to formalize nD-Laplace and introduce modifications to improve the mechanisms for clustering.
...
\todo[inline]{proof \& formalize grid-remapping / discretization for nD-Laplace}
...
Increasing the number of grid cells can improve the utility, but this comes at the cost of significantly increased space complexity.
Also, when the number of dimensions increases, the number of grid cells grows quadratic. \newline

\subsubsection{Optimalization}
To extend Andres et al.'s approach to n-dimensional data, we need an efficient way to search points in an n-dimensional hypersphere.
