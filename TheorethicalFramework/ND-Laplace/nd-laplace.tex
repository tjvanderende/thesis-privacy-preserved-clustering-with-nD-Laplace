...
\end{enumerate}
\deleted{For each mechanism, we explain the equation for \gls{gi}, the mechanism, and the data truncation.}
\added{
...
\deleted{If we select a $u$ smaller then the max diameter $r_H$, we can satisfy $d_{euc}(x_n, x0) < r_H < u$. Which is according to the proof provided by Min et al.
...
\added{Now we provide the proof that extends the 3-dimensional version provided by Min et al for the above theorem. We begin by noting the similarities and adding explanations to various steps of the process.}
%Let $G_c$ be the cuboid provided by Min et al., where the sides are of unequal sizes: $u < v < h$. To simplify our analysis, we consider the hypercube $G_n$ to have equal sides, denoted as $v = v_1, v_2, v_3 ... v_{n-1}$, where $n$ is the number of dimensions.

\added{We set the upper bound and lower bounds to be $K_{\epsilon'}(x)(x')$ for each $n$-dimensional vector $x$ and $x'$, such that $d_{euc}(x), d_{euc}(x') \leq r_H$ for all $x, x' \in X$.
Similar to proof provided by Min et al., we consider a case where a coordinate is outside the $n$-cube: $d_{euc}(x_5, x_6) \geq v$. Let $V$ be the set of locations for which $x_0$ is the closest location in $G_n$ \citep{9646489}.} 
\added{Assume $v_n > r_H$ for all $n \in N$, where $N$ is the number of dimensions.}
%Please note that we excluded the variable $w$ from this calculation when compared to the original proof. This omission is due to our lack of consideration for device precision in the context of clustering.}
%This variable is defined in the original proof as the number of cuboids $B$ around $x_0$ when compensated for the device precision \citep{9646489}.  \newline}
%The original proof by Min et al. considers a compensation for the device's precision. To facilitate clustering, this property is not needed. Therefore, we omit $w$ from our proof.
\added{As with the 3-dimensional space, some sides contribute negatively or positively to $K_{\epsilon'}(x)(x')$. Both cases are provided by Min et al., and we modified them to n-dimensional data:
\begin{align}
K_{\epsilon'}(x_5)(x') \leq (1 + n \cdot e^{\epsilon' \cdot v}) \int ... \int_V{D_{\epsilon'}(x_5)(V)dv}
\end{align}
Because nD-Laplace considers a $n$-cube, we replace $3 \cdot e^{\epsilon' \cdot v}$ by $n \cdot e^{\epsilon' \cdot v}$.  Also, we replaced $h = v$ to support n-dimensional data. Instead of using three integrals for each side of the $G_c$, we used an $n$ amount for every side $v \in G_n$.  \newline
The negative side, including these modifications is defined as \cite{9646489}:
\begin{align}
(1 - n \cdot e^{\epsilon' \cdot v}) \int ... \int_V{D_{\epsilon'}(x_6)(V)dv \leq K_{\epsilon'}(x_6)(x')}
\end{align}
Combining both inequalities, we obtain \citep{9646489}:
\begin{align}
\int ... \int_V D_{\epsilon'}(x_5)(V)dv \leq e^{\epsilon'd_{euc}(x_5, x_6)} \int ... \int_V D_{\epsilon'}(x_6)(V)dv
\end{align}
}
\added{
Combining all the above steps, we derive the following inequality \citep{9646489}:
\begin{align}
K_{\epsilon'}(x_5)(x') \leq e^{\epsilon' \cdot d_{euc}(x_5, x')} K_{\epsilon'}(x_6)(x') \frac{1 + n \cdot e^{\epsilon' \cdot v}}{1 - n \cdot e^{\epsilon' \cdot v}}
\end{align}
}
\added{To satisfy \gls{gi} we have to choose a $\epsilon'$ to compensate for the cell width $v$. The formula ultimately ensures \gls{gi} \citep{9646489} (See Formula: \ref{theorem:nd-laplace}).
Because $d_x$-privacy is a generalisation of \gls{gi}, the proof extends to this notion \citep{fernandes_generalised_2019}.}

\added{Finally, we omit the proof for clustering since it is considered a post-processing step. Data generated using the mechanism proven to provide $d_x$-privacy also provides it for post-processing \citep{feyisetan_leveraging_2019}. } \newline
