...
\end{theorem}
\todo[inline]{What is x'?}
\todo[inline]{What is Z?}
...
\begin{figure}[H]
   \centering
  \includesvg[width=0.8\textwidth]{TheorethicalFramework/ND-Laplace/Images/master-thesis-Page-10}
...
\added{The $G_c$ that was introduced for 3D-Laplace can be extended to a \textit{n}-dimensions by providing a \textit{n}-cube, as it can be seen as a generalization of the 3D cuboid.} \footnote{https://mathworld.wolfram.com/Hypercube.html}.
\added{Due to this extension the \textit{n}-cube has $2^n$ sides and is defined as $G_n$ with an $n$ amount of sides.
In contrast to the cuboid $G_c$, we consider equal sides for the purpose of simplicity.} 

\added{The probability of a point $x \in X$ in $G_h$ can be described as the following, as we did for 2D-Laplace and 3D-Laplace as well (See Equations \ref{eq:3d-grid-probability}, \ref{eq:grid-probability}): 
\begin{equation}
    R_h(x) = { y \in R^+ \ | \ \forall x' \in G_h \cdot d_{euc}(y, x') \leq d_{euc}(y, x')}
\end{equation}
This can be seen as a region $R_h$ in which we want to preserve $\epsilon$-geo-indistinguishability \citep{9646489}.}
\added{Consequently, the maximum $r$, is within the region $R_h$ and defined as $r_{max}$. This means, we have to select a suitable $\epsilon'$ that depends on the length of the grid units of $G_h$ and $\epsilon$, in such a way that we preserve $\epsilon'-$geo-indistinguishability for $r_{max}$ \citep{DBLP:journals/corr/abs-1212-1984, 9646489}.}

\added{The theorem for this is provided as the following:}
\begin{theorem}
Assume  $r_{max} = v \in G_h$. Let $\epsilon \in R+$ and $\epsilon' = \frac{\epsilon}{|G_h|}$such that: \\
  $\epsilon' \leq \epsilon$ \\
  Then $K^T_{\epsilon'}$ satisfies $\epsilon-d_{euc}$-privacy in any area of diameter $r_{max}$, namely: \\ 
  $K_{\epsilon'}(x)(Z) \leq e^{\epsilon \cdot d_{euc} (x, x')} \cdot K_{\epsilon'}(x')(Z) \ for \ all \ x, x' \in X, Z \subseteq X$.  \\
  Whenever $d_{euc}(x, z)$ and $d_{euc}(x, x') \leq r_{max}$.
  \label{theorem:nd-laplace-truncation}
\end{theorem}
\added{
Given the above theorem, the privacy of a data-point is protected in an area $A \cap G_h$:
\begin{equation}
z' = 
\begin{cases}
        z & \text{if} \ z \notin A \cap G_h \\ 
        min  & \text{if} \ x \in [-C, l(t_i) \cup r(t_i), C]
    \end{cases}    
\end{equation}
The $min(z', A \cap G_h)$ indicates a point $A$ closest to $z'$, which is provided by the high privacy budget $\epsilon'$. 
}
...
\added{Now we provide the proof, by redefining the basics of \gls{gi}, after which we explaining the theorem in detail.}
\added{In the fundamentals of \gls{gi}, it's essential to ensure that the likelihood of reporting a point within a specific region around $x$ when the true locations are $x_0$ and $x_0'$ should only differ at most $e^{\epsilon \cdot d(x_0, x_0')}$ \citep{DBLP:journals/corr/abs-1212-1984}. This occurs when the noise function causes the probability of generating a point near $x$ to drop significantly as you move further from the true location $x_0$.
The area around $x_0$ is defined as the radius $r$, so according to \gls{gi} any point that is generated within this area preserves $\epsilon$-geo-indistinguishability. The probability of this, was expressed using an intergral to represent the area between $0$ and $r$ \citep{DBLP:journals/corr/abs-1212-1984}}:
\begin{equation}
    C_\epsilon(r) = \int^r_0 D_{\epsilon, R(p)dp}
\end{equation}
\added{Where the integral represents the area of $r$, and $p$ is a random value between the original point $x_0$ and $r$. This $p$ is generated according to the earlier description we gave for 2D-Laplace (See Algorithm \ref{alg:2d-laplace}}).
\added{The proof 2D of \gls{gi} was provided by Andres et al., and was extended to 3D-Laplace and nD-Laplace by Min et al. and Fernandes et al., respectively. 
The latter, extended the proof for nD-Laplace by showing that nD-Laplace can be represented by selecting multiple spherical coordinates $(r, \theta)$ independently. Hence, the proof they provided is an extension of that of 2D-Laplace. In addition to considering the area of $r$ for a planar, they also consider the area for the angular coordinates over the hyper sphere \citep{fernandes_generalised_2019}}. \added{This is expressed as the product of two integrals.}\newline \newline

\added{
Therefore, we should assume that $r_{max} \leq v_n \forall \ v_n \in G_h$. Given the grid-unit sizes of $G_h$ are the same, it is sufficient to say $r_{max} \leq v$.
The limitation on $r_{max}$ is crucial, as it determines the region where $\epsilon$-geo-indistinguishability can be guaranteed \citep{9646489}. Therefore, the proof shows there is a suitable $\epsilon'$ based on $\epsilon$ and $v$ that satisfies privacy for the area $r_{max}$. 
Let $R_h(x) = S_h$ be a region, which is a grid unit $v$ around $x_0$ of $G_h$. We first recap the proof provided by Andres et al. for 2-dimensional data, where the probability $x$ is $S_2$ and $r = d(x_0, x)$ \citep{DBLP:journals/corr/abs-1212-1984}: 
\begin{equation}
     \int_{S_2} D_\epsilon(x_0)(x)ds_2 \leq e^{\epsilon \cdot d(x_0, x_0')} \leq \int_{S_2} D_\epsilon(x_0')(x)ds_2
\end{equation}

Because the nD-Laplace is an extension of 2D-Laplace in the way we select separate $(r, \theta_n)$ for all dimensions $n$, the proof can be extended to:
\begin{equation}
    \int ... \int_{S_h} D_{\epsilon}(x_0)(x)d_{euc}s_h \leq e^{\epsilon \cdot d(x_0, x_0')} \ \leq \int ... \int_{S_h} D_{\epsilon}(x_0')(x)d_{euc}s_h
\end{equation}
}
Where we define the product of multiple integrals for each side of $S_h$. 
For the above, we need to provide two things: $r' = d(x_0', x)$ and $ r \leq r_{max}$ to make sure $r'$ is always $ \leq v$ and contained by $G_h$. 
Second, we want to make sure the point $x$ is remapped to the closest point in $A \cap G_h$. Therefore, the $\epsilon$ is modified based on the grid unit $v$. For this we apply theorem 3.2 by Andres et al., where the combined mechanism $K$ also satisfies $\epsilon$-geo-indistinguishability for $\epsilon = \sum_i{\epsilon_i}$. 
Thus, the new epsilon value $\epsilon'$ for all $v \in G_h$  is defined as $\epsilon' = \frac{\epsilon}{|G_h|}$. 
Because we are not considering the finite precision, we are be-able to preserve $\epsilon$-geo-indistinguishability for the truncated variant $K = K^T$ using $\epsilon'$ with $d(x_0', x) \leq r_{max}$ for every $x \in S_h$. \newline
...
Based on the above, we are also be-able to provide the theorem for truncated $K$:
\added{$K = K^T$, where $K^T$ is the truncated variant of the probability $K$:
\begin{theorem}
    If $r_{max} \geq diam(A)$, then $K^T$ provides $\epsilon'-d_x$-privacy, namely
    $K^T(x_0)(x) \leq e^{\epsilon' \cdot d_{euc} (x_0, x_0')} \cdot K^T(x')(Z)$  for all $x_0, x_0' \in A$. 
\end{theorem}
...
\todo[inline]{Needs to be updated (probably move.}
