...
Similar to Min et al., we consider a case where a coordinate is outside the hypercube: $d_{euc}(x_5, x_6) \geq v$. Let $V$ be the set of locations for which $x_0$ is the closest location in $G_n$ \citep{9646489}.} 
\added{Each side $v$ should be larger than $r_H$ while considering precision coordinates: $v_n > r_H$ for all $n \in N$, where $N$ is the number of dimensions. 
The original proof by Min et al. considers a compensation for the device's precision.  To facilitate clustering, this property is not needed. Therefore, we omit $w$ from our proof. This variable is defined as the number of cuboids $B$ around $x_0$ when compensated for the device precision \citep{9646489}.  \newline}
\added{We extend the negative impact on $K$ in 3-dimensional space to n-dimensional space with slight modifications as provided by Min et al.:
\begin{align}
K_{\epsilon'}(x_5)(x') \leq (1 + n \cdot e^{\epsilon' \cdot v}) \int ... \int_V{D_{\epsilon'}(x_5)(V)dv}
\end{align}
Because we consider a $n$-cube, we replace $3 \cdot e^{\epsilon' \cdot v}$ by $n \cdot e^{\epsilon' \cdot v}$.  Also, we replaced $h = v$ to support n-dimensional data. Instead of using three integrals for each side of the $G_c$, we used an $n$ amount for every side $v \in G_n$.  \newline
