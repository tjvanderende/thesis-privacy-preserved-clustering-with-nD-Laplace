...
Although this approach remaps data within the original domain of $X$, it is not guaranteed it preserves $G$ anymore.
...
So, the $R_X(x)$ has a different shape depending on the distance $x_0$ and $x'$ (ergo, it depends on the grid cell width).
The probability may be 0, and \gls{gi} cannot be satisfied.
\newpage
To overcome this issue, Andres et al. propose a way of calculating $\epsilon'$, depending on the grid-cell width $u \cdot w$. \citep{DBLP:journals/corr/abs-1212-1984}.
...
\end{theorem}
Here, $\delta_{\theta}$ is the machine's precision. \todo[inline]{Do we need to do something with this still?}
The $u$ is the smallest grid-cell width; this could differ from $w$, but we assume $u = w$.
...
So, $r_{max}$ is the maximum distance between points in $X$.

In the context of our research, it is more convenient to find the $u$ based on a given $\epsilon$.
