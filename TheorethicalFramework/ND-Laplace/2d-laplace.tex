...
\textbf{Calculating $r$:}
This variable is described as $dist(x_0, z)$ and can be randomly drawn by inverting the CDF \todo{show CDF} for the Laplace distribution:
\begin{equation}
...
If noise is added to it, it can be ensured that this data falls outside this range (figure \ref{figure:truncation-2d}).
\begin{figure}[ht]
...
If a user is located in a crowded area, the radius can be smaller than if the user is located in a rural area.
Because the user location is indistinguishable due to the overlap of other users' locations.
Instead of providing \gls{gi}, the authors introduce a more flexible privacy definition $d_x$-privacy.
The $r$ is calculated based on the mass of other locations in the region $r$, they call \emph{privacy mass}.
So, the total mass of a set A is defined as $M(A) = \sum_{x \in A} m(x) = a + q(x)b$.
Where $m(x)$ is the mass of a location $x$ and a value between 0 and 1 and $a$ is the number of points assigned to each location.
...
Although the method is an interesting approach to increasing utility while not reducing privacy it is hard to adopt for this research.
