...
\todo[inline]{Replace radius($\epsilon$) with different symbol}
...
\begin{enumerate}
    ...
    \item \gls{sc} \citep{rousseeuw_silhouettes_1987} This metric is similar in measuring cohesion within and separating clusters.
          ...
          A score of -1 indicates incorrect clustering and +1 for dense clusters \citep{rousseeuw_silhouettes_1987}.
          \todo[inline]{Research correct terminology for dense clusters}
    \item Davies-Bouldin (DB) \citep{davies_cluster_1979} uses the average distance between centroids. A lower score indicates good clustering.
          \todo[inline]{Centroids are for K-Means, also add for other algorithms}
\end{enumerate}
...
\todo[inline]{What to do with this? it seems he beginning of this section you already presented some literature studies,
    it is a bit confusing to bring the literature two times}
...
\begin{enumerate}
    ...
    \item \textbf{Adjusted Mutual Information (External validation)}:
          ...
          \end{equation}
          \todo[inline]{What is the input?}
          \todo[inline]{Formula needs explaination}
\end{enumerate}
