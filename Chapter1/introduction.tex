...
An extension of this mechanism, 3D-Laplace, added support for indoor navigation by including the altitude \citep{9646489}.
...
To our knowledge, \gls{gi} has never been applied for data clustering.
Therefore, the primary research objective of this study is to extend the use of \gls{gi} to apply it for the clustering of data. \newline

This thesis aims to developing a new privacy framework for training clustering algorithms on distributed n-dimensional data.
...
\begin{enumerate}
  \item We aim to create a new privacy framework named nD-Laplace using the three variants of nD-Laplace to train privacy-preserving clustering algorithms on distributed n-dimensional data.
  \item In addition to K-means, we demonstrate the working of our method on multiple other clustering algorithms.
  \item We extend the proof and implement truncation for 2D-Laplace and 3D-Laplace  to nD-Laplace. 
  \item We introduce an utility optimization technique for truncation with 2D-Laplace, 3D-Laplace and nD-Laplace.
...
To acquire this qualitative data, we conduct a series of experiments that include both cluster utility and mechanism utility/privacy experiments. These experiments are conducted using real-world datasets and synthetic datasets.
